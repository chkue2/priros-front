<template>
  <NuxtLayout name="mypage" now-page="payment-quick">
    <div class="payment-common">
      ...
      <p class="payment-warning">간편결제는 대표자 명의의 결제수단으로 등록해야 합니다.(법인카드는 대표자 기명카드로 등록)</p>
      <div class="payment-list">
        <div class="table-column">
          <div class="table-header payment-date">결제일</div>
          <div class="table-header payment-requester">요청자</div>
          <div class="table-header payment-alias">별칭</div>
          <div class="table-header payment-method">결제수단</div>
          <div class="table-header payment-info">결제정보</div>
          <div class="table-header payment-memo">내용</div>
        ...
        <div v-for="i in 5" :key="i" class="table-column column-box">
          <div class="table-contents payment-date">2023-04-05<br>10:27:40</div>
          <div class="table-contents payment-requester">loverly_kkk</div>
          <div class="table-contents payment-alias">업무계좌</div>
          <div class="table-contents payment-method">계좌번호</div>
          <div class="table-contents payment-info">110****984445</div>
          <div class="table-contents payment-memo">배상책임보험</div>
      ...
      <div class="payment-list-buttons">
        <button class="button--blue" @click="toggleRetroactivePaymentModal">취소분 소급결제</button>
        <div>
          <button class="button--darkgray">간편결제 등록</button>
          <button class="button--darkgray">간편결제 관리</button>
          <button class="button--gray" @click="togglePasswordPaymentModal">비밀번호 변경/재설정</button>
        </div>
      </div>
      <Pagination />
    </div>
    <RetroactivePaymentModal v-if="isRetroactivePaymentModalShow" @close-modal="toggleRetroactivePaymentModal" />
    <PaymentPasswordModal v-if="isPaymentPasswordModalShow" @close-modal="togglePasswordPaymentModal" />
  </NuxtLayout>
</template>
