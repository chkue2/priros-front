  ...
  <div id="wrapper" class="logined">
    ...
    <div class="mypage-wrapper">
      <nav class="mypage-nav">
        <NuxtLink to="/user/mypage/info" class="nav-home-door" :class="{active: nowPage === 'info'}">회원(사업자)정보</NuxtLink>
        <NuxtLink to="/user/mypage/user-info" class="nav-user" :class="{active: nowPage === 'user-info'}">사용자 정보</NuxtLink>
        <NuxtLink to="/user/mypage/user-list" class="nav-group" :class="{active: nowPage === 'user-list'}">사용자 관리</NuxtLink>
        <NuxtLink to="/user/mypage/setting" class="nav-setting" :class="{active: nowPage === 'setting'}">환경설정</NuxtLink>
        <NuxtLink to="/user/mypage/payment/quick" class="nav-book" :class="{active: isPaymentPage}">결제 등록 및 관리</NuxtLink>
        <div v-if="isPaymentPage" class="mypage-submenu">
          <NuxtLink to="/user/mypage/payment/quick" :class="{active: nowPage === 'payment-quick'}">간편결제내역</NuxtLink>
          <NuxtLink to="/user/mypage/payment/regular" :class="{active: nowPage === 'payment-regular'}">일반결제내역</NuxtLink>
        </div>
        <NuxtLink to="/user/mypage/expert-management" class="nav-badge" :class="{active: nowPage === 'expert-management'}">협약 전문가 관리</NuxtLink>
        <NuxtLink to="/user/mypage/inquire/list" class="nav-pencil" :class="{active: nowPage === 'inquire'}">1:1 문의</NuxtLink>
      </nav>
      <slot />
    </div>
  </div>
