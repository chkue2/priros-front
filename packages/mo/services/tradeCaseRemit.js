...
export const tradeCaseRemit = {
  async get(tradeCaseId) {
    const endpoint = getEndpoint(API_URL.TRADE_CASE.REMIT, {trade_case_id: tradeCaseId})
  ...
  async request(tradeCaseId, data) {
    const endpoint = getEndpoint(API_URL.TRADE_CASE.REMIT_REQUEST, {trade_case_id: tradeCaseId})
    return await POST_AUTH(endpoint, data)
  },
