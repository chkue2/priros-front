  ...
  <NuxtLayout name="dialog-header" headerTitle="프로필카드 보내기">
    <div class="send-profile-form">
      ...
      <input
        v-model="form.name"
        type="text"
        class="send-profile-input"
        placeholder="매수인 이름을 입력하세요"
      />
      <input
        v-model="form.mobile"
        type="tel"
        class="send-profile-input"
        placeholder="매수인 휴대폰번호를 입력하세요 (-제외)"
      />
    </div>
    <div class="send-profile-intro">
      <p>
        <b>'프로필카드 보내기' 란?</b><br />
        : 직접 관리하는 계약을 등기소다에 등록하여 편하게 안내 및 관리할 수 있는
        서비스<br /><br />
      </p>

      <p>
        - 우리 사무소와 계약을 진행하는 매수인에게 사무소의 프로필카드를 보내
        매수인의 계약정보 입력을 요청해 주세요.<br />
      </p>
      <p>
        - 매수인이 발송된 프로필카드를 확인하고 계약정보를 입력하면 사건을
        수임하여 확정해 주세요.<br />
      </p>
      <p>
        - 수임을 확정하면 매수인은 실시간으로 업무의 현황을 확인할 수 있어
        별도의 안내를 드리지 않아도 돼요!<br />
      </p>
    ...
    <div class="send-profile-buttons">
      <CommonBottomButton
        ...
        height="60px"
        width="50%"
        @handler-click-button="handlerClickCloseButton"
      />
      <CommonBottomButton
        id="send-button"
        text="보내기"
        ...
        height="60px"
        width="50%"
        @handler-click-button="handlerClickSendButton"
      />
    </div>
  </NuxtLayout>
</template>
...
const isValidate = computed(
  () => form.value.name !== "" && form.value.mobile !== ""
);
