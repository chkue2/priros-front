<template>
  <div class="inji-issuance-container">
    ...
    <p class="inji-subtitle">
      1% 할인된 금액으로 정산은 하루에 한번만!<br />프리로스에서 제공하는
      서비스를 이용해보세요.
    </p>
    <div class="inji-form">
      ...
      <select
        v-model="price"
        v-show="!isPriceInputShow"
        id="priceSelect"
        class="inji-form-select"
      >
        <option value="" disabled>선택</option>
        <option
          v-for="(amount, i) in amountTypeEnums"
          :key="i"
          :value="amount[0]"
        >
          {{ amount[1] }}
      ...
      <div v-show="isPriceInputShow" class="inji-form-input-container">
        <input
          ...
          ref="priceInputElm"
          type="text"
          class="inji-form-input"
          placeholder="발행금액 직접입력"
          @keyup.enter="priceInputBlur"
          @blur="priceInputBlur"
        />
      </div>
      ...
      <select v-model="selectName" class="inji-form-select">
        <option
          v-for="(client, i) in clientTypeEnums"
          :key="i"
          :value="client[0]"
        >
          {{ client[1] }}
      ...
      <div class="inji-form-input-container">
        <input
          v-model="name"
          type="text"
          class="inji-form-input w-105"
          :placeholder="namePlaceholder"
          :readonly="selectName === 'default'"
        />
        <input
          v-model="num"
          type="text"
    ...
    <p class="inji-help-text">
      • 발행취소는 당일 PM 05:30까지 마이페이지에서 가능
    </p>
  ...
  <div class="inji-issuance-buttons">
    ...
    <button class="button--apply" @click="handlerClickApplyButton">
      발행요청
    </button>
  </div>
</template>
...
const namePlaceholder = computed(() =>
  selectName.value === "firm" ? "상호명 입력" : "이름 입력"
);
const numPlaceholder = computed(() =>
  selectName.value === "firm" ? "사업자등록번호 입력" : "생년월일 입력"
);
...
const handlerClickApplyButton = () => {
  if (price.value === "" || price.value === "0") {
    alertStore.open("발행금액을 선택해주세요.");
    return false;
  }
  if (selectName.value === "firm") {
    if (name.value === "") {
      alertStore.open("상호명을 입력해주세요.");
      return false;
    }
    if (num.value.length !== 10) {
      alertStore.open("사업자등록번호 10자리를 -없이 입력해주세요.");
      return false;
    }
  }
  if (selectName.value === "client") {
    ...
    if (num.value.length !== 8) {
      alertStore.open("생년월일 8자리를 -없이 입력해주세요. (YYYYMMDD)");
      return false;
    }
  }
  emit("inji-generate", {
